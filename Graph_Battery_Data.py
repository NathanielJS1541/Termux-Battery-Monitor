...
if (args.stop_time is not None) and (args.stop_percentage != 100):
    console.print(
        "[WARN] Both a percentage and time have been specified. Whichever is"
...
with progress_bar:
    while current_progress < 100:
        ...
        with subprocess.Popen(
            ...
            battery_progress = int(
                (
                    (current_status["percentage"] - battery_initial)
                    / (args.stop_percentage - battery_initial)
                )
                * 100
            )
            ...
            progress_bar.update(
                total_progress, advance=current_progress - last_progress
            )
...
if len(current) > 0:
    ...
    console.print(
        f"[INFO] Saving output to {args.output_file.resolve()}", style="green"
    )
